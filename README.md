...
CapCut/Dreamina, BytePlus и CDN-домены Сетки добавлены в ранние PROXY-исключения, чтобы их API и CDN не попадали в обход локальных fake-IP адресов.

`setka.ru` добавлен в ручные DIRECT-исключения, потому что сервис от hh.ru может плохо открываться при маршрутизации через VPN. CDN-домены Сетки идут через PROXY отдельно, потому что картинки и ассеты могут плохо грузиться напрямую.
